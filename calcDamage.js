...
function calcDamage(Player, Enemy){
    ...
    damage = wp / (1 + (Enemy.armor*(1-Player.wpPene/100)) / 100);
...
function calcBSstack(Player, Enemy, stack){
    if(stack > 8){
	stack = 8;
    }
    Enemy.armor = Enemy.armor * (1 - stack*0.05 - Player.wpPene/100);
    return stack;
}
...
function calcDamage2(){
    ...
    // 装備コスト表示------------------------------
    var node = document.getElementById("PlayerCost");
    node.innerHTML = Player.cost.toString(10) + "ゴールド";
    node = document.getElementById("EnemyCost");
    node.innerHTML = Enemy.cost.toString(10) + "ゴールド";
    //--------------------------------------------
	...
	for(var i = 0; i < numatk; i++){
	    ...
	    // ボーンソー処理
	    if(Player.BSflg == 1){
		var BSstack = calcBSstack(Player, Enemy, i);
	...
	// スキルレベル選択の表示
	document.getElementById("AL").onchange = function(){skillDamage(Player,Enemy)};
	document.getElementById("BL").onchange = function(){skillDamage(Player,Enemy)};
	document.getElementById("ULTL").onchange = function(){skillDamage(Player,Enemy)};
	 
	if (Player.deflg){
	    var node = document.getElementById("DragonEye");
	    node.innerHTML = "";
	    document.getElementById("DE").style.display = "inline-block";
	    addInput(node, "range", "DEstack", 0, 9, 0);
	    document.getElementById("DEstack").onchange = function(){skillDamage(Player,Enemy)};
	}
	else{
	    document.getElementById("DE").style.display = "none";
	    document.getElementById("DragonEye").innerHTML = "";
	}
	...
	if (Player.SFflg){   // スペルファイアを装備している時のダメージ表示
	    var node = document.getElementById("SF");
	    displayInline("SF");
	    var SFdamage = ApplyDamageEquation(Player.cp, Player.cpPene, Enemy.shield);
	    node.innerHTML = "スペルファイアダメージ:&emsp;" + SFdamage.toFixed(1).toString(10) + "ダメージ";
	}
	else{
	    displayOff("SF");
